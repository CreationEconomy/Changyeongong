...
#define _DUTY_DEGREE (_DUTY_MAX - _DUTY_MIN)/180

// global variables
float timeout; // unit: us
float dist_min, dist_max, dist_raw, dist_prev, dist_ema; // unit: mm
float degree = (_DUTY_MAX - _DUTY_MIN)/180;
...
// get a distance reading from the USS
  ...
  dist_ema = dist_raw*_DIST_ALPHA + dist_ema*(1-_DIST_ALPHA);
...
// adjust servo position according to the USS read value
  ...
  if(dist_ema <= 180) {
    ...
    myservo.writeMicroseconds(_DUTY_MIN + int((dist_ema-180) * degree));
